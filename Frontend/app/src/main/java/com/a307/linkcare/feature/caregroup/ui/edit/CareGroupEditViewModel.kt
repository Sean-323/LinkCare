...
@HiltViewModel
class CareGroupEditViewModel @Inject constructor(
    ...
    fun updateGroup(
        ...
        viewModelScope.launch {
            ...
            val result = repo.updateGroup(
                ...
                imageFile = imageFile,
                isSleepRequired = isSleepRequired,
                isWaterIntakeRequired = isWaterIntakeRequired,
                isBloodPressureRequired = isBloodPressureRequired,
                isBloodSugarRequired = isBloodSugarRequired,
                minCalorie = null,
                minStep = null,
                minDistance = null,
                minDuration = null
            ...
            _updateState.value = result.fold(
                ...
                onFailure = { UpdateState.Error(it.message ?: "Failed to update group") }
            )
        }
    }
    ...
    fun delegateLeader(groupSeq: Long, newLeaderUserSeq: Long) {
        viewModelScope.launch {
            ...
            result.fold(
                onSuccess = {
                    Log.d("CareGroupEditViewModel", "그룹장 위임 성공")
                ...
                onFailure = { error ->
                    Log.e("CareGroupEditViewModel", "그룹장 위임 실패: ${error.message}")
    ...
    fun kickMember(groupSeq: Long, targetUserSeq: Long) {
        viewModelScope.launch {
            ...
            result.fold(
                onSuccess = {
                    Log.d("CareGroupEditViewModel", "그룹원 내보내기 성공")
                    _memberActionState.value = MemberActionState.KickSuccess("그룹원을 내보냈습니다")
                    // 그룹 상세 정보 다시 로드 (백그라운드에서)
                    refreshGroupDetailSilently(groupSeq)
                },
                onFailure = { error ->
                    Log.e("CareGroupEditViewModel", "그룹원 내보내기 실패: ${error.message}")
    ...
    private suspend fun refreshGroupDetailSilently(groupSeq: Long) {
        try {
            ...
            Log.e("CareGroupEditViewModel", "그룹 상세 정보 갱신 실패: ${e.message}")
        }
    }
    ...
    fun leaveGroup(groupSeq: Long) {
        viewModelScope.launch {
            ...
            result.fold(
                onSuccess = {
                    Log.d("CareGroupEditViewModel", "그룹 탈퇴 성공")
                    _memberActionState.value = MemberActionState.LeaveSuccess("그룹에서 탈퇴했습니다")
                },
                onFailure = { error ->
                    Log.e("CareGroupEditViewModel", "그룹 탈퇴 실패: ${error.message}")
    ...
    fun deleteGroup(groupSeq: Long) {
        viewModelScope.launch {
            ...
            result.fold(
                onSuccess = {
                    Log.d("CareGroupEditViewModel", "그룹 삭제 성공")
                    _memberActionState.value = MemberActionState.DeleteSuccess("그룹을 삭제했습니다")
                },
                onFailure = { error ->
                    Log.e("CareGroupEditViewModel", "그룹 삭제 실패: ${error.message}")
